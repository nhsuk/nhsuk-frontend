...
class ConfigurableComponent extends component.Component {
  ...
  constructor($root, config) {
    ...
    this.constructor;
    if (!index.isObject(childConstructor.schema)) {
      throw new index$1.ConfigError(index.formatErrorMessage(childConstructor, 'Config passed as parameter into constructor but no schema defined'));
    }
    ...
    const errors = validateConfig.validateConfig(schema, this.config);
    if (errors[0]) {
      throw new index$1.ConfigError(index.formatErrorMessage(childConstructor, errors[0]));
    }
  }
