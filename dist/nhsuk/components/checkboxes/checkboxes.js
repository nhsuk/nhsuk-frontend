 ...
 */
class Checkboxes extends component.Component {
  /**
  ...
  syncConditionalRevealWithInputState($input) {
    index$1.toggleConditionalInput($input, 'nhsuk-checkboxes__conditional--hidden');
  }
  ...
  unCheckExclusiveInputs($input) {
    ...
    const allInputsWithSameNameAndExclusiveBehaviour = document.querySelectorAll(`input[type="checkbox"][data-checkbox-exclusive][name="${$input.name}"]`);
...
function initCheckboxes(options = {}) {
  ...
  const $checkboxes = $scope.querySelectorAll(`[data-module="${Checkboxes.moduleName}"]`);
