...
const options = {
  ...
  variant: {
    type: 'string',
    required: false,
    description: 'Optional variant of card – `"feature"`, `"primary"`, `"secondary"`, `"warning"`, `"non-urgent"`, `"urgent"` or `"emergency"`.',
    released: '10.4.0'
  },
  type: {
    type: 'string',
    required: false,
    description: 'Optional type of care card – `"non-urgent"`, `"urgent"` or `"emergency"`. Replaced by the `variant` option.',
    released: '6.0.0',
    deprecated: '10.4.0'
  },
  feature: {
  ...
  primary: {
    type: 'boolean',
    required: false,
    description: 'If set to `true`, the card will become a primary card variant (with chevron).',
    released: '7.1.0',
    deprecated: '10.4.0'
  },
  secondary: {
    type: 'boolean',
    required: false,
    description: 'If set to `true`, the card will become a secondary card variant.',
    released: '7.1.0',
    deprecated: '10.4.0'
  },
  warning: {
    type: 'boolean',
    required: false,
    description: 'If set to `true`, then the card will become a warning card variant used by the warning callout.',
    released: '10.3.0',
    deprecated: '10.4.0'
  },
  imgURL: {
    type: 'string',
    required: false,
    description: 'The URL of the image in the card. Replaced by the `image.src` option.',
    released: '4.0.0',
    deprecated: '10.3.0'
  },
  imgALT: {
    type: 'string',
    required: false,
    description: 'The alternative text of the image in the card. Replaced by the `image.alt` option.',
    released: '4.0.0',
    deprecated: '10.3.0'
  },
  image: {
    ...
    params: {
      ...
      html: {
        type: 'string',
        required: false,
        description: 'HTML to use for the image content. If `html` is provided, the `src` and `alt` arguments will be ignored.',
        released: '10.3.0'
      }
    }
  },
      ...
      items: {
        ...
        params: {
          ...
          element: {
            type: 'string',
            required: false,
            description: 'HTML element for the action item – `"button"` or `"a"`. In most cases you will not need to set this as it will be configured automatically if `href` is provided.',
            released: '10.6.0'
          },
          text: {
            type: 'string',
          ...
          name: {
            type: 'string',
            required: false,
            description: 'Name for the `button`. This has no effect on `a` elements.',
            released: '10.6.0'
          },
          type: {
            type: 'string',
            required: false,
            description: 'Type of `button` – `"button"`, `"submit"` or `"reset"`. Defaults to `"submit"`. This has no effect on `a` elements.',
            released: '10.6.0'
          },
          value: {
            type: 'string',
            required: false,
            description: 'The `value` attribute for the `button`. This has no effect on `a` elements.',
            released: '10.6.0'
          },
          href: {
            type: 'string',
            required: true,
            description: 'The URL that the action item should link to. If this is set, `element` will be automatically set to `"a"` if it has not already been defined.',
            released: '10.3.0'
          },
          classes: {
            type: 'string',
  ...
  caller: {
    type: 'nunjucks-block',
    required: false,
    description: 'Not strictly a parameter but a Nunjucks code convention. Using a `call` block enables you to call a macro with all the text inside the tag. This is helpful if you want to pass a lot of content into a macro. To use it, you will need to wrap the entire card component in a `call` block.',
    released: '9.5.0'
  },
  classes: {
    type: 'string',
