 ...
 */
class CharacterCount extends component.Component {
  /**
  ...
  constructor($root) {
    ...
    if (!$textarea || !($textarea instanceof HTMLTextAreaElement || $textarea instanceof HTMLInputElement)) {
    ...
    const $fallbackLimitMessage = document.getElementById(fallbackLimitMessageId);
     ...
     */
    this.config = Object.assign({}, CharacterCount.defaults, CharacterCount.getDataset(this.$root));
   ...
   */
  isOverThreshold() {
    var _this$config$threshol;
    // Determine the remaining number of characters/words
    const currentLength = this.count(this.$textarea.value);

    // Set threshold if presented in config
    const thresholdPercent = (_this$config$threshol = this.config.threshold) != null ? _this$config$threshol : 0;
...
function initCharacterCounts(options = {}) {
  ...
  const $characterCounts = $scope.querySelectorAll(`[data-module="${CharacterCount.moduleName}"]`);
