...
class CharacterCount extends configurableComponent.ConfigurableComponent {
  ...
  constructor($root, config = {}) {
    ...
    if (countType === 'characters' || countType === 'words' && maxwords === undefined || countFunction) {
      ...
      if (!this.segmenter && !countFunction) {
        throw new index.SupportError(index$1.formatErrorMessage(CharacterCount, 'Support for "Intl.Segmenter" required'));
      }
    }
  ...
  configOverride(datasetConfig) {
    ...
    if ('maxwords' in datasetConfig || maxwords !== undefined) {
      console.warn(index$1.formatErrorMessage(CharacterCount, 'Option `maxwords` is deprecated. Use `maxlength` with `countType: "words"` instead.'));
