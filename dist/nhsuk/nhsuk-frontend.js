...
class Table extends ConfigurableComponent {
  ...
  sort(index, direction = 'ascending') {
    ...
    this.$rows.sort(($rowA, $rowB) => {
      ...
      const [valueA, isNumericA] = direction === 'ascending' ? this.getValue($cellA) : this.getValue($cellB);
      const [valueB, isNumericB] = direction === 'ascending' ? this.getValue($cellB) : this.getValue($cellA);
      return isNumericA && isNumericB ? this.collators.number.compare(valueA, valueB) : this.collators.string.compare(valueA, valueB);
    });
  }
  ...
  updateScreenReaderStatusMessage(index, direction = 'ascending') {
    const $header = this.$headers[index];
    this.$screenReaderStatusMessage.textContent = this.i18n.t('sortAnnouncement', {
  ...
  getValue($cell) {
    ...
    const hasFormatNumeric = $cell.classList.contains(`${headerClass}--numeric`) || $cell.classList.contains(`${cellClass}--numeric`);
    const output = normaliseString(sortValue);
    const numeric = hasFormatNumeric || typeof output === 'number';
    return [(_output$toString = output == null ? void 0 : output.toString()) != null ? _output$toString : '', numeric];
  }
