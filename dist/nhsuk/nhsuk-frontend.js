...
class CharacterCount extends ConfigurableComponent {
  ...
  constructor($root, config = {}) {
    ...
    if (countType === 'characters' || countType === 'words' && maxwords === undefined || countFunction) {
  ...
  configOverride(datasetConfig) {
    ...
    if ('maxwords' in datasetConfig || maxwords !== undefined) {
      console.warn(formatErrorMessage(CharacterCount, 'Option `maxwords` is deprecated. Use `maxlength` with `countType: "words"` instead.'));
      if (!('maxlength' in datasetConfig)) {
        var _datasetConfig$maxwor;
        configOverrides.maxlength = (_datasetConfig$maxwor = datasetConfig.maxwords) != null ? _datasetConfig$maxwor : maxwords;
  ...
  formatCountMessage(remainingNumber, countType) {
    countType = countType != null ? countType : this.config.countType;
...
class Scroll extends ConfigurableComponent {
  ...
  update() {
    ...
    // Toggle right shadow
    $root.classList.toggle(`${Scroll.moduleName}--scrollable-right`, $content.scrollLeft < $content.scrollWidth - $content.clientWidth);
    if (isOverflowing) {
      this.$content.setAttribute('tabindex', '0');
    } else {
      this.$content.removeAttribute('tabindex');
