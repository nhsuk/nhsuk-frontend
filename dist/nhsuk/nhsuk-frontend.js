...
function normaliseString(value, property) {
  ...
  if (!isValid(value) || outputType && !['string', 'number', 'boolean'].includes(outputType)) {
    return;
  }
...
class SupportError extends NHSUKFrontendError {
  ...
  constructor(scopeOrMessage = document.body) {
    let supportMessage = 'noModule' in HTMLScriptElement.prototype ? 'NHS.UK frontend initialised without `<body class="nhsuk-frontend-supported">` from template `<script>` snippet' : 'NHS.UK frontend is not supported in this browser';
    if (!isScope(scopeOrMessage)) {
      supportMessage = 'NHS.UK frontend initialised without `<script type="module">`';
    }
...
class ConfigurableComponent extends Component {
  ...
  constructor($root, config) {
    ...
    if (!isObject(childConstructor.schema)) {
      throw new ConfigError(formatErrorMessage(childConstructor, 'Config passed as parameter into constructor but no schema defined'));
    }
...
class CharacterCount extends ConfigurableComponent {
  ...
  constructor($root, config = {}) {
    ...
    if (countType === 'characters' || countType === 'words' || countFunction) {
      ...
      if (!this.segmenter && !countFunction) {
        throw new SupportError(formatErrorMessage(CharacterCount, 'Support for "Intl.Segmenter" required'));
      }
    }

    // Determine the count function to use
    this.countFunction = countFunction != null ? countFunction : CharacterCount.countFunctions[countType];
    ...
    this.maxLength = maxlength != null ? maxlength : Infinity;
   ...
   */
  updateCount(text) {
    this.length = this.countFunction.call(this, text != null ? text : this.$textarea.value, this.countFunctionContext);
  }
 ...
 * @import { Schema } from '../../common/configuration/index.mjs'
 */
CharacterCount.countFunctions = Object.freeze({
  /**
  ...
  characters(text, context) {
    return context.segmenter ? Array.from(context.segmenter.segment(text)).length : 0;
  },
  /**
   * Count words
   *
   ...
   */
  words(text, context) {
    const segments = context.segmenter ? Array.from(context.segmenter.segment(text)) : [];
...
class Checkboxes extends ConfigurableComponent {
  ...
  checkInputs($input, behaviour = 'exclusive') {
    ...
    for (const $behaviourInput of $behaviourInputs) {
      ...
      if ($groupInputs.every($groupInput => $groupInput.checked)) {
  ...
  getGroup($input, behaviour) {
    ...
    return !('behaviourGroup' in dataset) && behaviour === 'exclusive' ? dataset.checkboxExclusiveGroup : dataset.behaviourGroup;
  }
  ...
  getGroupSelector($input, behaviour) {
    ...
    // Check for deprecated `data-checkbox-*` group attributes
    return !('behaviourGroup' in dataset) && behaviour === 'exclusive' ? `[data-checkbox-${behaviour}-group="${behaviourGroup}"]` : `[data-behaviour-group="${behaviourGroup}"]`;
  }
  ...
  getGroupInputs($behaviourInput, behaviour) {
    ...
    const $groupInputs = document.querySelectorAll(`input[type="checkbox"]${groupSelector}`);
  ...
  getBehaviourInputs($input, behaviour) {
    ...
    const $behaviourInputs = document.querySelectorAll(`input[type="checkbox"][data-behaviour="${behaviour}"]${groupSelector}, input[type="checkbox"][data-checkbox-${behaviour}]${groupSelector}`);
    return Array.from($behaviourInputs).filter($behaviourInput => $input.form === $behaviourInput.form);
  }
  ...
  setInputState($input, checked, behaviourGroup) {
    var _dataset$behaviourGro;
    const {
      dataset
    } = $input;
    const group = (_dataset$behaviourGro = dataset.behaviourGroup) != null ? _dataset$behaviourGro : dataset.checkboxExclusiveGroup;
  ...
  handleClick(event) {
    ...
    const isExclusive = dataset.behaviour === 'exclusive' || 'checkboxExclusive' in dataset;
    const isInclusive = dataset.behaviour === 'inclusive';
    if (isExclusive) {
...
class Code extends ConfigurableComponent {
  ...
  static checkSupport($root, config) {
    ...
    // Check for copy button support
    if (!($root != null && $root.classList.contains(`${Code.moduleName}--button`))) {
      return;
    }
...
class Header extends ConfigurableComponent {
  ...
  setupNavigation() {
    ...
    if ('ResizeObserver' in window) {
      this.resizeObserver = new window.ResizeObserver(() => window.dispatchEvent(new Event('resize')));
  ...
  onClick(event) {
    ...
    if (!$root.contains($target) || $target.closest('a, button') && !($menuToggle != null && $menuToggle.contains($target))) {
      this.closeMenu();
    }
  }
  ...
  resizeMenu() {
    ...
    this.updateNavigationTimer = window.requestAnimationFrame(this.handleUpdateNavigation);
  }
 ...
 */
class Scroll extends ConfigurableComponent {
  /**
   * @param {Element | null} $root - HTML element to use for component
...
class Table extends ConfigurableComponent {
  ...
  constructor($root, config = {}) {
    ...
    const $headers = this.$head ? Array.from(this.$head.querySelectorAll(`th.${headerClass}`)) : [];
    if ($headers.some($header => $header.querySelector('a')) || !$headers.some($header => 'sort' in $header.dataset)) {
      return this;
    }
    if ($headers.length && !$headers.every($header => $header instanceof HTMLElement)) {
      throw new ElementError({
        component: Table,
    ...
    const $rows = this.$body ? Array.from(this.$body.querySelectorAll(`.${rowClass}`)) : [];
    ...
    this.$root.insertAdjacentElement('beforebegin', this.$screenReaderStatusMessage);
    this.setup();
  }
  setup() {
    const {
    ...
    this.setupButtons();
    const $header = $headers.find($header => $header.matches('[aria-sort="ascending"], [aria-sort="descending"]'));
  ...
  sort(index, direction = 'ascending') {
    ...
    if (!(direction === 'ascending' || direction === 'descending') || !collators) {
      return;
    }
    $rows.sort(($rowA, $rowB) => {
  ...
  update(index, direction = 'ascending') {
    ...
    if (!(direction === 'ascending' || direction === 'descending') || !$body || !$headers.length || !$screenReaderStatusMessage) {
      return;
    }
    ...
    $screenReaderStatusMessage.textContent = this.formatStatusMessage(index, direction);
  }
  ...
  getValue(index, $row) {
    ...
    const value = normaliseString(sortValue != null ? sortValue : innerText);
...
function initAll(scopeOrConfig = {}) {
  ...
  const components = /** @type {const} */[[Button, config.button], [CharacterCount, config.characterCount], [Checkboxes, config.checkboxes], [Code, config.code], [ErrorSummary, config.errorSummary], [FileUpload, config.fileUpload], [Header, config.header], [NotificationBanner, config.notificationBanner], [PasswordInput, config.passwordInput], [Radios, config.radios], [Scroll, config.scroll], [SectionNavigation, config.sectionNavigation], [SkipLink, config.skipLink], [Table, config.table], [Tabs, config.tabs]];
 ...
 * @property {ComponentConfig<typeof SectionNavigation>} [sectionNavigation] - Section navigation config
...
export { Button, CharacterCount, Checkboxes, Code, Component, ConfigError, ConfigurableComponent, ElementError, ErrorSummary, FileUpload, Header, I18n, InitError, NHSUKFrontendError, NotificationBanner, PasswordInput, Radios, Scroll, SectionNavigation, SkipLink, SupportError, Table, Tabs, closestAttributeValue, createAll, extractConfigByNamespace, formatErrorMessage, getBreakpoint, initAll, isInitialised, isObject, isScope, isSupported, mergeConfigs, normaliseArray, normaliseDataset, normaliseOptions, normaliseString, setFocus, toggleConditionalInput, validateConfig, version };
