import { componentNameToMacroName } from './names.mjs';
import { renderMacro, macro as macro$1 } from './nunjucks/index.mjs';
...
function render(component, options) {
  const macroName = componentNameToMacroName(component, options?.prefix);
 ...
 */
function macro(component, options) {
  const macroName = componentNameToMacroName(component, options?.prefix);
  const macroPath = `${component}/macro.njk`;

  return macro$1(macroName, macroPath, options)
}
