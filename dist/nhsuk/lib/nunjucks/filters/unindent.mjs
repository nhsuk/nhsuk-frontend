...
function unindent(code) {
  ...
  const indents = lines.filter(line => /^\s+/.test(line)).flatMap(line => /^\s+/.exec(line)?.[0] ?? []);
  ...
  const indent = indents.reduce((a, b) => a.length <= b.length ? a : b, indents[0]);
...
export { unindent };
//# sourceMappingURL=unindent.mjs.map
