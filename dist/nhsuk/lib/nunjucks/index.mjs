...
export { configure } from './environment.mjs';
 ...
 */
function renderParams(context) {
  const params = context ? [context].flat() : [];
  const paramsFormatted = params.map(param => JSON.stringify(param, undefined, 2)).join(', ');
...
function macro(macroName, macroPath, options) {
  ...
  if (options?.context && Object.keys(options.context).length) {
    ...
    macroCall = prettier.format(`${macroName}(${paramsFormatted})`, {
      parser: 'espree',
      semi: false,
      singleQuote: false,
      trailingComma: 'none'
  ...
  macroString += options?.callBlock ? `{% call ${macroCall} -%}\n\n${options.callBlock.trim()}\n\n{%- endcall %}` : `{{ ${macroCall} }}`;
  return macroString;
}
...
export { env, macro, renderMacro, renderParams, renderString, renderTemplate, template };
