...
function uniqueHTML(html, index) {
  if (index) {
    ...
    .replaceAll(/ (aria-label|href|id|for|name)="([^"]*)"/g, (match, name, value) => ` ${name}="${uniqueValue(name, value, index)}"`)

    // Append unique ID to attribute value(s)
    .replaceAll(/ (?:data-)?(aria-controls|aria-describedby|aria-labelledby)="([^"]*)"/g, (match, name, values) => ` ${name}="${uniqueValues(name, values, index)}"`);
  }
  return html;
}
...
function uniqueValue(name, value, index) {
  if (!value || name === 'id' && value === 'maincontent' || name === 'href' && ['#', '#maincontent'].includes(value)) {
    return value;
  }
  if (name === 'aria-label') {
    return `${value} ${index}`;
...
function uniqueValues(name, values, index) {
  return values.split(' ').map(value => uniqueValue(name, value, index)).join(' ');
}

exports.uniqueHTML = uniqueHTML;
//# sourceMappingURL=unique-html.js.map
