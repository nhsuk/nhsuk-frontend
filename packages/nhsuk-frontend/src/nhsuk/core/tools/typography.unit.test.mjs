...
describe('Typography tools', () => {
  ...
  describe('@mixin nhsuk-typography-common', () => {
    it('should output a @font-face declaration by default', async () => {
      const sass = outdent`
        ...
        :root {
          @include nhsuk-typography-common($font-family: Helvetica);
        }
      `
      ...
      await expect(results).resolves.toMatchObject({
        css: expect.stringContaining('font-family: "Frutiger W01"')
      })
    })
    ...
    it('should not output a @font-face declaration when the user has changed their font', async () => {
      const sass = outdent`
        ...
        $app-font-family-monospace: monospace;
        ...
        :root {
          @include nhsuk-typography-common($font-family: $app-font-family-monospace);
        }
      `
      ...
      await expect(results).resolves.toMatchObject({
        css: expect.not.stringContaining('font-family: "Frutiger W01"')
      })
    })
