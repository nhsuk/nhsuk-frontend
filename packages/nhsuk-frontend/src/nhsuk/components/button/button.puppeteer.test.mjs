...
describe('Button', () => {
  beforeAll(async () => {
    await page.goto(`${BASE_URL}/components/button/as-a-link.html`)
  })
  ...
  describe('Button as a link', () => {
    it('triggers the click event when the space key is pressed', async () => {
      ...
      expect(url.href).toBe(`${BASE_URL}/components/button/as-a-link.html#`)
    })
  })
  ...
  describe('preventing double clicks', () => {
    ...
    describe('using data-attributes', () => {
      beforeEach(async () => {
        await page.goto(
          `${BASE_URL}/components/button/with-double-click-prevented.html`
        )
        await trackClicks()
      })
